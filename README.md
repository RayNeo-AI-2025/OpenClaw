...
- [依赖说明](#依赖说明)
- [海外用户指引（International Users）](#海外用户指引international-users)
- [License](#license)
...
| 服务 | 用途 | 获取方式 |
|------|------|----------|
| 阿里云 DashScope | Paraformer 实时语音识别 | 中国大陆：[控制台申请](https://dashscope.console.aliyun.com/)；海外：[International Console](https://bailian.console.alibabacloud.com/) |
| OpenClaw 智能体网关 | LLM 流式对话 | 自行部署（见下方） |

> **海外用户注意**：中国大陆版（aliyun.com）和国际版（alibabacloud.com）的 API Key **不互通**，请根据所在地区选择对应平台注册。国际版注册无需中国手机号。详见 [海外用户指引](#海外用户指引international-users)。
...
```properties
# 阿里云 DashScope — 语音识别
# 中国大陆用户：https://dashscope.console.aliyun.com/
# 海外用户：https://bailian.console.alibabacloud.com/
DASHSCOPE_API_KEY=sk-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx

# 海外用户须取消注释并使用国际版端点（国内用户无需修改）
...
| 字段 | 说明 |
|------|------|
| `DASHSCOPE_API_KEY` | 阿里云 DashScope 控制台获取（国内版与国际版 Key 不互通） |
| `DASHSCOPE_WS_ENDPOINT` | 海外用户须设为 `wss://dashscope-intl.aliyuncs.com/api-ws/v1/inference`；国内用户留空即可 |
...
| `DASHSCOPE_WS_ENDPOINT` | DashScope WebSocket 端点（海外用户须改为国际版端点） | 否 |
...
DASHSCOPE_WS_ENDPOINT=wss://dashscope-intl.aliyuncs.com/api-ws/v1/inference  # 海外用户须设置
...
## 海外用户指引（International Users）

OpenClaw 使用阿里云 DashScope 提供语音识别服务。DashScope 同时提供中国大陆版和国际版，两者的 API Key 和端点**互不相通**。

### 区别对比

| 项目 | 中国大陆版 | 国际版 |
|------|-----------|--------|
| 注册网站 | [aliyun.com](https://www.aliyun.com/)（需中国手机号） | [alibabacloud.com](https://www.alibabacloud.com/)（海外手机或邮箱即可） |
| 产品名称 | DashScope / 百炼 | Model Studio（DashScope 已合并入 Model Studio） |
| 控制台 | [dashscope.console.aliyun.com](https://dashscope.console.aliyun.com/) | [bailian.console.alibabacloud.com](https://bailian.console.alibabacloud.com/) |
| WebSocket 端点 | `wss://dashscope.aliyuncs.com/api-ws/v1/inference` | `wss://dashscope-intl.aliyuncs.com/api-ws/v1/inference` |
| 计费货币 | 人民币（CNY） | 美元（USD） |

### 海外用户配置步骤

1. 在 [alibabacloud.com](https://www.alibabacloud.com/) 注册账号（无需中国手机号）
2. 进入 [Model Studio 控制台](https://bailian.console.alibabacloud.com/) → API-KEY 管理，创建 API Key
3. 在 `openclaw.conf`（或 `local.properties`）中配置：
...
> **重要**：国际版 API Key 必须搭配国际版端点使用，反之亦然。混用会导致认证失败。
